...
/// @notice Optimized single-contract blockchain gemstone provenance system
contract GemChain {
    ...
    function addGovOfficer(address officer)
    external
    onlyAdmin
    {
        if (officer == address(0)) revert ZeroAddress();

        if (isGovOfficer[officer]) {
            revert AlreadyGovOfficer();
        }
    ...
    function removeGovOfficer(address officer)
    external
    onlyAdmin
    {
        if (!isGovOfficer[officer]) {
            revert OfficerNotFound();
        }
    ...
    function verifyGem(uint256 gemId)
    external
    onlyGovOfficer
    {
        ...
        if (gem.status != VerificationStatus.Pending) {
            revert GemNotPending();
        }
    ...
    function rejectGem(uint256 gemId)
    external
    onlyGovOfficer
    {
        Gem storage gem = gems[gemId];
    ...
    returns (uint256 transferId)
    {
        ...
        if (gem.currentOwner != msg.sender) {
            revert NotGemOwner();
        }

        if (gem.status != VerificationStatus.Verified) {
            revert GemNotVerified();
        }

        if (buyer == address(0)) revert ZeroAddress();

        if (!isOwner[buyer] || buyer == msg.sender) {
            revert InvalidBuyer();
        }

        if (activeTransferByGem[gemId] != 0) {
    ...
    function buyerConfirm(uint256 transferId)
    external
    {
        TransferRequest storage t = _getActiveTransfer(
            transferId
        );
    ...
    function sellerConfirm(uint256 transferId)
    external
    {
        TransferRequest storage t = _getActiveTransfer(
            transferId
    ...
    function _getActiveTransfer(
        uint256 transferId
    )
    internal
    view
    returns (TransferRequest storage t)
    {
        ...
        if (t.cancelled) {
            revert TransferAlreadyCancelled();
        }
    }

    function _tryFinalizeTransfer(
        uint256 transferId
    )
    internal
    {
